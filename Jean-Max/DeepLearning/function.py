...
def generate(input,generalOutput,name,model,preprocess):
    if not os.path.exists(generalOutput):
        os.makedirs(generalOutput)
    specificOutput=os.path.join(generalOutput, name)
    if not os.path.exists(specificOutput):
        os.makedirs(specificOutput)

    for imageName in os.listdir(input):
        imagePath=os.path.join(input, imageName)
        ...
        np.savetxt(os.path.join(specificOutput, imageName.split(".")[0]), feature)
