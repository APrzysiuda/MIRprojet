...
def globalFeatureExtraction(dataPath, featurePath, featureName):
    specificFeaturePath = os.path.join(featurePath, featureName)
    if not os.path.isdir(specificFeaturePath):
        os.mkdir(specificFeaturePath)
    for imageName in os.listdir(dataPath):
        image = cv2.imread(os.path.join(dataPath, imageName))
        feature = featureExtraction(image, featureName)
        imageName = imageName.split(".")[0]
        imageFeaturePath = os.path.join(specificFeaturePath, imageName + ".txt")
        np.savetxt(imageFeaturePath, feature)
