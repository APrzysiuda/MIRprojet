...
class Distance:
    def __init__(self, featurePath, requestName, imageName, distanceName):
        ...
        self.distance = int(distance_f(self.imageFeature, self.requestFeature, distanceName))
...
class Evaluation:
    def __init__(self, name, RT, RR50, RR100, RR500):
        self.name = name
        ...
        if listDistance[i].brand == requestBrand:
            ...
            if i <= 500:
                ...
                if i <= 100:
                    ...
                    if i <= 50:
                        RR50 += 1
    eval = Evaluation(name, RT, RR50, RR100, RR500)
    return eval


def evaluation1(listDistance, requestBrand):
    total = 0
    T = len(listDistance)
    for i in range(T):
        if listDistance[i].brand == requestBrand:
